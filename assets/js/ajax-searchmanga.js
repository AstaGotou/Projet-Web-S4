...
async function loadMangaSearch () {

    const container = document.querySelector('section')
    if (getNameFromURL() != null) {
        console.log(getNameFromURL());
        const response = await fetch('../../api/search-mangas.php?name=' + getNameFromURL());
        ...
        for (let i = counter; i < counter + 5; i++) {
            ...
            'beforeend',
            `<a href="manga.php?id=${liste[i].id}">
                <article>
                        <img src="../assets/img/poster/manga/${liste[i].chemin_photo}" alt="poster">
...
button.addEventListener('click', loadMangaSearch)

window.addEventListener('load', loadMangaSearch)
